...
// Saki-chan 双模型角色分工
// QWEN = Programmer：专注于代码实现，严格遵循接口规范
// DSR1 = Engineer & Architect：负责分析、规划、架构设计与调试
...
struct AnalysisResponse {
    ...
    #[serde(default)]
    task_type: String,
    /// new_project | modify_project | document | analysis | other
    #[serde(default)]
...
fn is_doc_step(step: &PlanStep) -> bool {
    !step.needs_compile
        && !step.files_to_create.is_empty()
        && step.files_to_create.iter().all(|f| {
...
fn build_doc_prompt(
    ...
    let target = step.files_to_create.first().map(|s| s.as_str()).unwrap_or("output.md");
    format!(
        "{role}{mental_model}请根据以下任务撰写文档，输出到 `{target}`。\n\n\
原始需求：{request}\n\
项目规则：{rules}\n\n\
任务说明：\n{submodule_prompt}\n\n\
使用以下格式输出（{lang}，严禁输出 .rs 或其他源代码文件）：\n\
```markdown filename=\"{target}\"\n\
（文档正文，Markdown 格式）\n\
```",
        role = ROLE_DSR1,
        mental_model = mental_model_section(analysis_doc),
...
{{\"understanding\":\"brief understanding in {lang}\",\"task_type\":\"natural language description\",\"task_category\":\"modify_project\",\"complexity\":5,\"gathered_info\":[{{\"label\":\"目标\",\"value\":\"...\",\"source\":\"需求推断\"}}],\"clarifications\":[{{\"question\":\"q\",\"recommendation\":\"suggestion\"}}]}}\n\n\
Rules:\n\
- task_type is free-form natural language, NOT an enum\n\
- task_category MUST be exactly one of: \"new_project\"(从零创建新项目), \"modify_project\"(修改/扩展现有代码), \"document\"(文档/报告/论文写作), \"analysis\"(代码/需求/数据分析), \"other\"(其他)\n\
- complexity is 1-10\n\
...
请基于已读取的所有文件内容来撰写，确保信息准确完整。",
    ...
    let (cat_icon, cat_label) = match analysis.task_category.as_str() {
        "new_project"     => ("🏗️", "新建项目"),
        "modify_project"  => ("✏️",  "修改项目"),
        "document"        => ("📄", "文档写作"),
        "analysis"        => ("🔍", "分析研究"),
        _                 => ("💬", "通用任务"),
    };
    println!("{CYAN}🎯 {cat_icon} {cat_label}  {GRAY}│{RESET}  {CYAN}复杂度 {}/10{RESET}", analysis.complexity);
    if !analysis.task_type.is_empty() {
        println!("{GRAY}   {}{RESET}", analysis.task_type);
    }
    if !analysis.understanding.is_empty() {
        println!("{GRAY}   💡 {}{RESET}", analysis.understanding);
    }
    if !analysis_doc.is_empty() {
        println!("{GRAY}   📝 分析文档已生成 ({} chars){RESET}", analysis_doc.len());
    }
...
描述：{task_type}\n\n\
规划规则（必须遵守）：\n\
- new_project / modify_project → 输出 .rs/.py 等源码文件，needs_compile 按需设置\n\
- document → 输出 .md/.txt 文档文件，needs_compile 必须 false，NEVER 创建 .rs 文件\n\
- analysis → 输出 .md 分析报告，needs_compile 必须 false，NEVER 创建 .rs 文件\n\
- other → 根据具体需求判断，优先输出文档\n\n\
...
- All text in {lang}",
    ...
    for step in &plan.steps {
        ...
        println!("  {GREEN}{}. {}{RESET} {kind}  {GRAY}[{}]{RESET}",
            step.id, step.name, step.assigned_model);
        for f in &step.files_to_create {
            let status = if work_dir.join(f).exists() {
                format!("{YELLOW}修改{RESET}")
            } else {
                format!("{GREEN}新建{RESET}")
            };
            println!("     {GRAY}→ {f}  [{status}{GRAY}]{RESET}");
        }
    }

    // Warn when planner lists source files in non-compile steps (common small-model error).
    {
        const SRC_EXTS: &[&str] = &["rs", "py", "cpp", "c", "go", "js", "ts", "java"];
        for step in &plan.steps {
            if !step.needs_compile {
                let bad: Vec<&str> = step.files_to_create.iter()
                    .filter(|f| SRC_EXTS.iter().any(|e| f.ends_with(&format!(".{e}"))))
                    .map(|s| s.as_str())
                    .collect();
                if !bad.is_empty() {
                    println!("{YELLOW}  ⚠ 步骤 '{}' 含源码文件但未标记编译 → 写入将被阻止: {}{RESET}",
                        step.name, bad.join(", "));
                }
            }
        }
    ...
    // Confirmation gate — always ask unless /edit is active (persistent "don't ask again").
    if !edit_mode {
        print!("{CYAN}▶ 执行 {} 个步骤? {GRAY}[y=执行 / n=取消 / a=始终执行(同/edit)]: {RESET}",
            plan.steps.len());
        let _ = io::stdout().flush();
        ...
        match ans.trim().to_lowercase().as_str() {
            "a" | "always" => {
                state.lock().unwrap().edit_mode = true;
                println!("{GREEN}✓ 已切换为始终执行模式，之后不再询问（/edit 可关闭）{RESET}");
            }
            "y" | "yes" | "" => {
                // proceed this time only
            }
            _ => {
                println!("{GRAY}已取消。{RESET}");
                return Ok(());
            }
        }
    }
    ...
    for (step_idx, step) in plan.steps.iter().enumerate() {
        ...
        // ── Doc step fast-path: simplified prompt, skip code review ──────
        if is_doc_step(step) {
            let doc_prompt = build_doc_prompt(step, &rules, user_request, &output_lang, &analysis_doc);
            let spinner = Spinner::new(SpinnerState::Crafting, Arc::clone(&global_tokens), Arc::clone(&global_start));
            let (doc_raw, usage_c) = ollama.chat(exec_model, &doc_prompt, Some(&exec_opts))?;
            add_global_tokens(&global_tokens, &usage_c);
            spinner.stop();
            record_usage(state, &usage_c);

            let target = step.files_to_create.first().cloned().unwrap_or_else(|| "output.md".to_string());
            let mut blocks = extract_code_blocks(&doc_raw);
            // Remap source-extension filenames (fallback detection error) to the actual target
            const SRC_EXTS_DOC: &[&str] = &["rs","py","cpp","c","go","js","ts","java"];
            for block in &mut blocks {
                if SRC_EXTS_DOC.iter().any(|e| block.1.ends_with(&format!(".{e}"))) {
                    block.1 = target.clone();
                }
            }
            if blocks.is_empty() {
                // Model skipped code fences — save trimmed raw output directly
                let fpath = work_dir.join(&target);
                if let Some(p) = fpath.parent() { let _ = fs::create_dir_all(p); }
                let old = fs::read_to_string(&fpath).unwrap_or_default();
                let trimmed = doc_raw.trim().to_string();
                let _ = fs::write(&fpath, &trimmed);
                print_code_diff(&target, &old, &trimmed);
                written_files.insert(target.clone(), trimmed);
                if !all_completed.contains(&target) { all_completed.push(target); }
            } else {
                write_or_patch_files(&blocks, &work_dir, &mut written_files, &mut all_completed, false);
            }
            continue; // Skip review for doc steps
        }
...
All comments and text in {lang}.",
            role = if exec_model == QWEN { ROLE_QWEN } else { ROLE_DSR1 },
            mental_model = mental_model_section(&analysis_doc),
            request = user_request,
            task_type = analysis.task_type,
            submodule_prompt = step.submodule_prompt,
        ...
        if blocks.is_empty() {
            ...
            write_or_patch_files(&blocks, &work_dir, &mut written_files, &mut all_completed, any_needs_compile);
        }
        ...
        for review_attempt in 1..=REVIEW_MAX_ATTEMPTS {
            ...
            if !fix_blocks.is_empty() {
                write_or_patch_files(&fix_blocks, &work_dir, &mut written_files, &mut all_completed, any_needs_compile);
            }
        }
    }
    ...
    if !written_files.is_empty() && any_needs_compile {
        ...
        for arch_iter in 1..=ARCH_MAX_ITERATIONS {
            ...
            if !rework_blocks.is_empty() {
                write_or_patch_files(&rework_blocks, &work_dir, &mut written_files, &mut all_completed, true);
            }
        }
    }
